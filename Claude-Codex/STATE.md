...
**当前焦点：M5 网页编辑器第一期（T-008）已出规格，等 Codex 认领。** 技术栈定了（ADR-015：Python+Flask 后端复用现有逻辑+JSON API，原生 HTML/JS 前端，本地运行，API 可替换）。第一期只做「文案审改」——网页改 plan.json 文案并存回、同步字幕/口播文本、改过的 scene 置 edited=true。施工图 `CONTRACTS/T-008_web_editor_phase1_spec.md`。
...
- [ ] M5: 审核 Web 界面 + 增量重渲染（改一段只重跑一段）← **进行中：第一期 T-008 文案审改已出规格**
- [ ] M6: 多平台版本输出（重渲染）
- [ ] M7: 发布 + 数据回流（能手动先手动）
