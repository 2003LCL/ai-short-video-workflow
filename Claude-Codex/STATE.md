...
> T-010 增量重渲染复审通过（含真实 TTS 端到端验证），DONE。M5 第一期+第二期均完整可用：双击启动→改文案→点重新生成→只重配改过段→出新视频。
## 一句话定位
...
**当前焦点：M5 第二期（T-010 增量重渲染）复审通过，DONE（Claude 2026-06-12，含真实联网 edge TTS 端到端验证）。** 「改文案→点重新生成→看到新视频」已闭环：网页「重新生成视频」按钮 → 只对 edited=true 的段重配音（实测 scene2/3 mp3 md5 改前改后一致、未被重配）→ 整条重渲 MP4 → 清 edited → 原子写 plan.json。失败回滚已验证（模拟 MP4 失败：旧 video.mp4/mp3 恢复、plan.json 不落盘）。
...
- [ ] M5: 审核 Web 界面 + 增量重渲染（改一段只重跑一段）← **进行中：第一期（T-008+T-009）+ 第二期（T-010 增量重渲染）均已 DONE；剩第三期投喂分页/可视化时间轴**
- [ ] M6: 多平台版本输出（重渲染）
- [ ] M7: 发布 + 数据回流（能手动先手动）
