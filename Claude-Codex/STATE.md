...
> T-011 网页离线投喂页复审通过，DONE。**M5 三期全部完成**，下一步折回 M4（素材打标签+自动匹配）。
## 一句话定位
...
**M5 全部三期已完成（核心闭环跑通）**：
- 第一期 T-008 文案审改 + T-009 一键启动（双击 start_editor.bat 起网页改文案存回）
- 第二期 T-010 增量重渲染（点「重新生成视频」只重配改过段、整条重渲 MP4，含真实 TTS 验证、失败回滚）
- 第三期 T-011 网页离线投喂页（展示 T-006 提示词+一键复制+大模型链接，贴回 AI 的 JSON 宽容解析校验写盘，零 key 也能用真实大模型生成文案）

**完整用户闭环**：双击启动 → （可选）离线生成文案/或已有文案 → 网页改文案存回 → 点「重新生成视频」→ 出带新文案/配音/字幕的 MP4。店主级非技术用户全程可用。

**当前焦点：M5 完整收尾，下一步折回 M4**（素材打标签+自动匹配，产品护城河/最大瓶颈，ADR-004）。等用户拍板启动 M4，PM 出规格。

**T-007 file provider 命令行版**：优先级低（T-011 网页版已覆盖主场景），留作无网页时备选。
...
- [x] M5: 审核 Web 界面 + 增量重渲染（改一段只重跑一段）✅ DONE（三期全完成：T-008 文案审改 + T-009 一键启动 + T-010 增量重渲染 + T-011 网页离线投喂页）
- [ ] M6: 多平台版本输出（重渲染）
- [ ] M7: 发布 + 数据回流（能手动先手动）
